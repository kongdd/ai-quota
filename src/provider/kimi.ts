...
/** 解析 Coding Plan API key：`KIMI_API_KEY` > `KIMI_CODING_API_KEY` > `MOONSHOT_API_KEY`。 */
export function resolveKimiApiKey(): string | undefined {
  return process.env.KIMI_API_KEY ?? process.env.KIMI_CODING_API_KEY ?? process.env.MOONSHOT_API_KEY;
}
