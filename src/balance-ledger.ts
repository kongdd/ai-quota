...
export function defaultLedgerPath(file = "api-usage.json"): string {
  return join(env.XDG_CONFIG_HOME ?? join(homedir(), ".config"), "ai-quota", file);
}
...
export function resolveBudget(raw: string | undefined, envKeys: string[], saved: number, name: string): number {
  const value = raw ?? envKeys.map((k) => env[k]).find(Boolean) ?? String(saved);
...
export function persistBudgetCaps(
  ...
  const currency = (caps.currency ?? saved?.currency ?? env.DEEPSEEK_CURRENCY ?? "CNY").toUpperCase();
  const state =
