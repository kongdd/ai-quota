...
import { KNOWN_PROVIDERS, isEnabled, loadAuthConfig, loadPiAuthKey, type KnownProvider } from "./core-auth.js";
import {
...
async function runDeepseek(values: QueryValues): Promise<ModelRemain[]> {
  const apiKey = loadPiAuthKey("deepseek") ?? loadPiAuthKey("deepseek-api") ?? env.DEEPSEEK_API_KEY;
